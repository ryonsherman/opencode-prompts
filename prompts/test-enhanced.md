...
```
project_profile()                           — Languages, frameworks, test setup
git_context()                               — Branch, dirty files, recent commits
memory_retrieve("test", scope: "all")       — Past testing work
decision_search("testing")                  — Past testing decisions
error_search("<relevant area>")             — Past errors that need test coverage
codebase_search("test OR spec OR describe") — Find existing test patterns
snippet_search("test")                      — Reusable test patterns
```
...
2. **Find existing tests**: Use `glob()` and `codebase_search()` to find test files
3. **Read the code**: Understand functions, classes, and their contracts
4. **Identify gaps**: What's tested, what's not
5. **Check error history**: Use `error_search()` to find bugs that need regression tests

### Phase 3: Categorize Test Needs
...
- Code related to past errors (from `error_search()`)
...
2. **Check `snippet_search("test")`** for reusable test patterns
3. **Create test file** using write tool if it doesn't exist (follow project conventions)
4. **Write the actual test code** — not pseudocode, not suggestions, real tests
5. **Write descriptive test names** that explain the scenario
6. **Follow AAA pattern**: Arrange, Act, Assert
7. **Include edge cases**: null, empty, boundary values, errors
8. **Save reusable patterns** with `snippet_save()` if the test pattern is generalizable
...
```
memory_store(
  content: "Added tests for <scope>. Created <N> test cases covering <summary>",
  tags: ["testing", "<project-area>"]
)
```

If you made a testing decision:
```
decision_log(
  title: "Use factory pattern for test fixtures",
  decision: "Adopted factory functions instead of raw fixtures for flexibility",
  context: "Test data setup was becoming repetitive and hard to maintain",
  tags: ["testing", "patterns"]
)
```

If you created a reusable test pattern:
```
snippet_save(
  title: "Mock async API response",
  code: "...",
  language: "typescript",
  tags: ["testing", "mock", "async"]
)
```
...
[Brief context from project_profile, past testing work, relevant decisions]
...
**Related**: [Link to past error if this is a regression test]
...
[List any test snippets saved for future use]
...
## Plugin Usage During Testing

### Finding Existing Tests

```
codebase_search("describe.*test")              — Find test suites
codebase_search("pytest.fixture")              — Find pytest fixtures
codebase_search("beforeEach.*mock")            — Find mock setup patterns
```

### Checking History

```
error_search("undefined")                      — Past null/undefined errors
error_search("timeout")                        — Past async issues
decision_search("mock")                        — Mocking decisions
memory_retrieve("test", scope: "all")          — Past testing notes
```

### Recording Findings

```
error_log(error_text: "Missing test for auth edge case", context: "Found during coverage analysis", tags: ["testing"])
decision_log(title: "...", decision: "...", context: "...", tags: ["testing"])
snippet_save(title: "...", code: "...", tags: ["testing"])
memory_store(content: "...", tags: ["testing"])
```
...
9. **Store significant findings** — future testing benefits from your discoveries
